...
# Restrict to your office/VPN CIDR(s) to reduce attack surface.
# Example: ["203.0.113.0/24", "198.51.100.5/32"]
# Required — no default so it must be set explicitly in each .tfvars file.
variable "eks_public_access_cidrs" {
  description = "CIDRs allowed to reach the EKS public API endpoint. Set to your VPN/office CIDR(s), not 0.0.0.0/0."
  type        = list(string)
}
