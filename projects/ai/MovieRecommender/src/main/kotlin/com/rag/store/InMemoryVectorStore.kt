...
/**
 * In-memory [VectorStore] implementation — the default backend.
 *
 * ## How it works
 *
 ...
 * ## Limitations
 *
 * Re-embeds the entire catalogue on every startup (~5–10s). Embeddings are not
 * persisted. For persistence, activate [QdrantVectorStore] via `VECTOR_STORE=qdrant`.
 *
 * @param ollama Client used to embed items and queries.
 */
class InMemoryVectorStore(private val ollama: OllamaClient) : VectorStore {
     ...
     *
     * Processes items one at a time rather than concurrently to avoid overwhelming Ollama.
     *
     * @param items The items to embed and store.
     */
    override suspend fun indexAll(items: List<MediaItem>) {
        println("Building in-memory vector index for ${items.size} items…")
